...
bool StateWrapper::setState(ompl::base::ScopedState<> state, const Eigen::VectorXd& value)
{
    if(value.size() != _size)
        return false;
...
bool StateWrapper::getState(const ompl::base::ScopedState<> state, Eigen::VectorXd& value)
{
    if(value.size() != _size)
        return false;
