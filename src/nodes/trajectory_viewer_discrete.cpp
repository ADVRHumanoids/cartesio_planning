...
//         rate = std::make_shared<ros::Rate>(1);
    ...
    auto sub = nh.subscribe<trajectory_msgs::JointTrajectory>("joint_trajectory_discrete", 1, on_trj_received);

    // custom robot state publisher
    XBot::Cartesian::Utils::RobotStatePublisher rspub(model);


    ros::Rate fixed_rate(1.);
    while(ros::ok())
    {
        if(rate)
        {
            ...
            // publish model tf
            rspub.publishTransforms(ros::Time::now(), "planner/discrete");
        }
        else
        {
            fixed_rate.sleep();
